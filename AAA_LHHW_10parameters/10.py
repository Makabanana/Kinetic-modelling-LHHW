...
# 模型名称
MODEL_NAME = "Model_E_integral_two_reaction_LHHW_10param_DE_10"

# 当前脚本所在文件夹
BASE_DIR = Path(__file__).resolve().parent

# 输出文件夹
OUT_DIR = BASE_DIR / "output" / "model_E_integral_10param_DE_10"
...
#
# KCO2, KCO, KH2O_H2 通过 exp 还原，所以一定大于 0
# DeltaHad_CO2_over_R, DeltaHad_CO_over_R, DeltaHad_H2O_H2_over_R 的边界全是负数
# 所以对应 DeltaHad 一定小于 0
...
BOUNDS = [
    ...
    (-20.0, 10.0),          # ln_KCO2_ref
    (-30000.0, -1e-9),      # DeltaHad_CO2_over_R

    (-20.0, 10.0),          # ln_KCO_ref
    (-30000.0, -1e-9),      # DeltaHad_CO_over_R

    (-20.0, 10.0),          # ln_KH2O_H2_ref
    (-30000.0, -1e-9),      # DeltaHad_H2O_H2_over_R
]
...
def load_data():
    ...
    # 不再筛选 GHSV
    # 原代码里的 df = df[df["GHSV"] == 12000].copy() 已删除
...
def fit_integral_model(df_group):
    """
    拟合 Model E integral LHHW 10 参数版本。
    """
    ...
    print("\n" + "=" * 60)
    print("开始拟合 Model E integral LHHW 10 参数版本")
    ...
    params_output.update(physical_constraints)

    print("\n模型 Model E integral LHHW 10 参数版本拟合完成")
    ...
    print("\n物理约束检查：")
    print("KCO2_ref > 0 =", physical_constraints["KCO2_ref_positive"])
    print("KCO_ref > 0 =", physical_constraints["KCO_ref_positive"])
    print("KH2O_H2_ref > 0 =", physical_constraints["KH2O_H2_ref_positive"])

    print("KCO2_T_all_positive =", physical_constraints["KCO2_T_all_positive"])
    print("KCO_T_all_positive =", physical_constraints["KCO_T_all_positive"])
    print("KH2O_H2_T_all_positive =", physical_constraints["KH2O_H2_T_all_positive"])

    print(
        "DeltaHad_CO2_over_R < 0 =",
        physical_constraints["DeltaHad_CO2_over_R_negative"],
    )
    print(
        "DeltaHad_CO_over_R < 0 =",
        physical_constraints["DeltaHad_CO_over_R_negative"],
    )
    print(
        "DeltaHad_H2O_H2_over_R < 0 =",
        physical_constraints["DeltaHad_H2O_H2_over_R_negative"],
    )
...
def save_results(summary, params, result_df):
    ...
    summary_df.to_excel(
        OUT_DIR / "model_E_integral_10param_summary.xlsx",
        index=False,
    )

    params_df.to_excel(
        OUT_DIR / "model_E_integral_10param_parameters.xlsx",
        index=False,
    )

    result_df.to_excel(
        OUT_DIR / "model_E_integral_10param_predictions.xlsx",
        index=False,
    )
    ...
    make_single_parity_plot(
        ...
        title="Model E Integral LHHW 10 Param CH3OH",
        save_path=OUT_DIR / "parity_CH3OH_model_E_integral_10param.png",
    )
    ...
    make_single_parity_plot(
        ...
        ylabel="Predicted CO",
        title="Model E Integral LHHW 10 Param CO",
        save_path=OUT_DIR / "parity_CO_model_E_integral_10param.png",
    )
    ...
    make_residual_vs_temperature_plot(
        ...
        title="Model E Integral LHHW 10 Param CH3OH residual vs T",
        save_path=OUT_DIR / "residual_vs_T_CH3OH_model_E_integral_10param.png",
    )
    ...
    make_residual_vs_temperature_plot(
        ...
        ylabel="Residual CO",
        title="Model E Integral LHHW 10 Param CO residual vs T",
        save_path=OUT_DIR / "residual_vs_T_CO_model_E_integral_10param.png",
    )
...
def main():
    ...
    print("\n==============================")
    print("Model E integral LHHW 10 parameter fitting")
    print("==============================")
