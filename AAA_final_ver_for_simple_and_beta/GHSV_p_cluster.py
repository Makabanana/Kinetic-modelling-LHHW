...
# 输出文件夹
OUT_DIR = Path("R_GHSV_P_cluster_improved")
...
def fit_one_ghsv_p_group(df_group, ghsv_value, p_value):
    ...
    make_parity_plot(
        ...
        title=f"Parity Plot for MeOH, GHSV = {ghsv_label}, p = {p_label_for_file} MPa",
    ...
    make_parity_plot(
        ...
        ylabel="Predicted CO",
        title=f"Parity Plot for CO, GHSV = {ghsv_label}, p = {p_label_for_file} MPa",
...
def main():
    ...
    for ghsv in ghsv_values:
        ...
        df_ghsv = df[np.isclose(df["GHSV"], ghsv)].copy()

        for p_value in p_values:

            df_group = df_ghsv[np.isclose(df_ghsv["p"], p_value)].copy()
            ...
            summary_df, params_df, result_df, beta_summary = fit_one_ghsv_p_group(
                df_group,
                ghsv,
                p_value
            )
