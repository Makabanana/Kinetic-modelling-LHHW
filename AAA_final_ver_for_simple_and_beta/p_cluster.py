...
# 输出文件夹
OUT_DIR = Path("R_P_cluster_improved")
...
# 3. beta 诊断
# =========================
def diagnose_beta(df_group, p_label, save_dir=None):
    ...
    if save_dir is not None:
        beta_df.to_excel(
            save_dir / f"p_{p_label}_beta_values.xlsx",
            index=False
        )
        beta_summary.to_excel(
            save_dir / f"p_{p_label}_beta_summary.xlsx",
            index=False
        )
...
# 10. 拟合单个 p 分组
# =========================
def fit_one_p_group(df_group, p_value):
    ...
    group_dir = OUT_DIR / f"p_{p_label_for_file}"
    group_dir.mkdir(exist_ok=True)

    print("\n" + "=" * 60)
    print(f"开始拟合 p = {p_label_for_file}")
    ...
    # 保存
    summary_df.to_excel(
        group_dir / f"p_{p_label_for_file}_summary.xlsx",
        index=False
    )

    params_df.to_excel(
        group_dir / f"p_{p_label_for_file}_parameters.xlsx",
        index=False
    )

    result_df.to_excel(
        group_dir / f"p_{p_label_for_file}_predictions.xlsx",
        index=False
    )
    ...
    make_parity_plot(
        ...
        title=f"Parity Plot for MeOH, p = {p_label_for_file} MPa",
        save_path=group_dir / f"p_{p_label_for_file}_parity_meoh.png"
    )
    ...
    make_parity_plot(
        ...
        ylabel="Predicted CO",
        title=f"Parity Plot for CO, p = {p_label_for_file} MPa",
        save_path=group_dir / f"p_{p_label_for_file}_parity_co.png"
    )
...
def main():
    ...
    for p_value in p_values:
        ...
        df_group = df[np.isclose(df["p"], p_value)].copy()

        if len(df_group) < 8:
            print(f"p = {p_value} 的数据点太少，跳过")
            continue

        summary_df, params_df, result_df, beta_summary = fit_one_p_group(
            df_group,
            p_value
        )
    ...
    all_summary_df.to_excel(
        OUT_DIR / "all_p_summary.xlsx",
        index=False
    )

    all_params_df.to_excel(
        OUT_DIR / "all_p_parameters.xlsx",
        index=False
    )

    all_predictions_df.to_excel(
        OUT_DIR / "all_p_predictions.xlsx",
        index=False
    )

    all_beta_summary_df.to_excel(
        OUT_DIR / "all_p_beta_summary.xlsx",
        index=False
    )
