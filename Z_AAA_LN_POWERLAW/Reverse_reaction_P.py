...
OUT_DIR = BASE_DIR / "Result_Reverse_reaction_P"
OUT_DIR.mkdir(exist_ok=True)

R = 8.314
eps = 1e-12
P_LIST = [3.0, 4.0, 5.0, 6.0, 7.0]
...
def load_data():
    ...
    print("原始总数据点数:", len(df))
    print("原始总压 p 唯一值:", sorted(df["p"].unique()))

    return df


def filter_data_by_p(df, p_target):
    dfp = df[np.isclose(df["p"], p_target, atol=1e-8)].copy()

    if dfp.empty:
        print(f"\n警告：筛选 p = {p_target} MPa 后没有数据，跳过。")
        return None

    print(f"\n==============================")
    print(f"固定总压 p = {p_target} MPa")
    print(f"剩余数据点数: {len(dfp)}")
    print("GHSV 分组:", sorted(dfp["GHSV"].unique()))
...
# 单组拟合
# =========================
def fit_one_cluster(dfc, ghsv_label, p_target, out_dir):
    ...
    df_out["GHSV_cluster"] = ghsv_label
    df_out["p_fixed"] = p_target
    df_out.to_excel(out_dir / f"fit_results_p{int(p_target)}MPa_{ghsv_tag}.xlsx", index=False)
    ...
    param_df.loc[len(param_df)] = ["p_fixed", p_target]
    ...
    param_df.to_excel(out_dir / f"fitted_parameters_p{int(p_target)}MPa_{ghsv_tag}.xlsx", index=False)
    ...
    print(f"\n拟合组别 = {ghsv_label} | p = {p_target} MPa")
...
# 画图
# =========================
def plot_parity_by_group(df_all, exp_col, pred_col, title, save_name, out_dir):
    plt.figure(figsize=(6, 6))

    for ghsv in sorted(df_all["GHSV_cluster"].unique(), key=lambda x: str(x)):
        part = df_all[df_all["GHSV_cluster"] == ghsv]
        plt.scatter(part[exp_col], part[pred_col], label=f"{ghsv}", alpha=0.7)
...
def plot_parity_single(df_one, exp_col, pred_col, title, save_name, out_dir):
    plt.figure(figsize=(6, 6))

    plt.scatter(df_one[exp_col], df_one[pred_col], alpha=0.7)

    min_val = min(df_one[exp_col].min(), df_one[pred_col].min())
    max_val = max(df_one[exp_col].max(), df_one[pred_col].max())
...
def run_one_pressure(df_all_raw, p_target):
    df = filter_data_by_p(df_all_raw, p_target)
    if df is None or df.empty:
        return None

    out_dir_p = OUT_DIR / f"p_{int(p_target)}MPa"
    ...
    for ghsv in sorted(df["GHSV"].unique()):
        ...
        res = fit_one_cluster(dfc, ghsv, p_target, out_dir_p)
        results.append({k: v for k, v in res.items() if k != "df"})
        all_df_grouped.append(res["df"])

    summary_grouped = pd.DataFrame(results)
    summary_grouped.to_excel(
        out_dir_p / f"comparison_summary_by_GHSV_p{int(p_target)}MPa.xlsx",
        index=False
    )

    df_grouped_all = pd.concat(all_df_grouped, ignore_index=True)
    df_grouped_all.to_excel(
        out_dir_p / f"all_grouped_predictions_p{int(p_target)}MPa.xlsx",
        index=False
    )

    # 2. 所有 GHSV 合并整体拟合
    res_global = fit_one_cluster(df.copy(), "ALL_GHSV", p_target, out_dir_p)
    summary_global = pd.DataFrame([{k: v for k, v in res_global.items() if k != "df"}])
    summary_global.to_excel(
        out_dir_p / f"global_fit_summary_p{int(p_target)}MPa.xlsx",
        index=False
    )

    df_global = res_global["df"]
    df_global.to_excel(
        out_dir_p / f"global_fit_predictions_p{int(p_target)}MPa.xlsx",
        index=False
    )

    # 3. 总汇总
    summary_all = pd.concat([summary_grouped, summary_global], ignore_index=True)
    summary_all.to_excel(
        out_dir_p / f"comparison_summary_grouped_plus_global_p{int(p_target)}MPa.xlsx",
        index=False
    )
    ...
    # 4. 作图
    plot_parity_by_group(
        df_all=df_grouped_all,
        exp_col="rMeOH",
        pred_col="rMeOH_pred",
        title=f"MeOH Parity Plot by GHSV at p={p_target} MPa",
        save_name=f"parity_plot_MeOH_by_GHSV_p{int(p_target)}MPa.png",
        out_dir=out_dir_p
    )
    ...
    plot_parity_by_group(
        ...
        exp_col="rCO",
        pred_col="rCO_pred",
        title=f"CO Parity Plot by GHSV at p={p_target} MPa",
        save_name=f"parity_plot_CO_by_GHSV_p{int(p_target)}MPa.png",
        out_dir=out_dir_p
    )
    ...
    plot_parity_single(
        ...
        exp_col="rMeOH",
        pred_col="rMeOH_pred",
        title=f"MeOH Global Parity Plot at p={p_target} MPa",
        save_name=f"parity_plot_MeOH_global_p{int(p_target)}MPa.png",
        out_dir=out_dir_p
    )
    ...
    plot_parity_single(
        ...
        exp_col="rCO",
        pred_col="rCO_pred",
        title=f"CO Global Parity Plot at p={p_target} MPa",
        save_name=f"parity_plot_CO_global_p{int(p_target)}MPa.png",
        out_dir=out_dir_p
    )

    summary_grouped["fit_scope"] = "grouped"
    summary_global["fit_scope"] = "global"

    return pd.concat([summary_grouped, summary_global], ignore_index=True)


def main():
    df_all_raw = load_data()

    all_pressure_summaries = []

    for p_target in P_LIST:
        res = run_one_pressure(df_all_raw, p_target)
        if res is not None:
            all_pressure_summaries.append(res)

    if all_pressure_summaries:
        final_summary = pd.concat(all_pressure_summaries, ignore_index=True)
        final_summary.to_excel(
            OUT_DIR / "summary_all_pressures.xlsx",
            index=False
        )
        print("\n所有压力循环完成")
        print(final_summary)
