# 按GHSV一起拟合，比较 simple power law 和 power law × (1 - beta)
# 没有引入Tave和lnA
import pandas as pd
import numpy as np
...
# =========================
# 7. 按 GHSV 分组画 parity plot
# =========================
def plot_parity_for_ghsv(ghsv, res_simple, res_beta):
    ...
    plt.title(f'Parity Plot for MeOH, GHSV = {ghsv}')
    plt.legend()
    plt.grid()
    plt.tight_layout()
    plt.savefig(f'parity_plot_meoh_GHSV_{int(ghsv)}.png', dpi=300)
    plt.show()
    ...
    plt.title(f'Parity Plot for CO, GHSV = {ghsv}')
    plt.legend()
    plt.grid()
    plt.tight_layout()
    plt.savefig(f'parity_plot_co_GHSV_{int(ghsv)}.png', dpi=300)
    plt.show()
