# 按照 GHSV 和 p 双重分类
# 只拟合显式可逆反应形式 reversible power law
# 不固定逆反应产物项级数
# 只画 parity plot
#
# 反应1:
# CO2 + 3H2 <-> CH3OH + H2O
#
# r1 = r1_forward - r1_reverse
# r1 = k1f * fCO2^n1_A * fH2^n1_B
#      - k1f / K1 * fCH3OH^n1_C * fH2O^n1_D
#
# 反应2:
# CO2 + H2 <-> CO + H2O
#
...
#      - k2f / K2 * fCO^n2_D * fH2O^n2_E
#
# 在 lnA 基础上引入 Tave 重参数化
...
# 输出文件夹
OUT_DIR = Path("R_GHSV_P_cluster_reversible_free_reverse")
...
# 不参与速率方程拟合
# =========================
def diagnose_beta(df_group, ghsv_label, p_label, save_dir=None):
    ...
    print(f"\nGHSV = {ghsv_label}, p = {p_label} 的 beta 诊断")
    print(
    ...
    if save_dir is not None:
        beta_df.to_excel(
            save_dir / f"GHSV_{ghsv_label}_p_{p_label}_beta_values.xlsx",
            index=False
        )
        beta_summary.to_excel(
            save_dir / f"GHSV_{ghsv_label}_p_{p_label}_beta_summary.xlsx",
            index=False
        )
...
# =========================
# 5. reversible power law
#
# 参数顺序:
# ln_k1f_ref, E1f, n1_A, n1_B, n1_C, n1_D,
# ln_k2f_ref, E2f, n2_A, n2_B, n2_D, n2_E
#
# 反应1:
# r1 = k1f * fCO2^n1_A * fH2^n1_B
#      - k1f / K1 * fCH3OH^n1_C * fH2O^n1_D
...
#      - k2f / K2 * fCO^n2_D * fH2O^n2_E
# =========================
def calc_predictions_reversible(par, fuga, temperature, Tave):
    ...
    r1_forward = k1f * (fCO2 ** n1_A) * (fH2 ** n1_B)
    r1_reverse = (k1f / K1) * (fCH3OH ** n1_C) * (fH2O ** n1_D)

    r2_forward = k2f * (fCO2 ** n2_A) * (fH2 ** n2_B)
    r2_reverse = (k2f / K2) * (fCO ** n2_D) * (fH2O ** n2_E)
...
# 用相对误差平方和
# =========================
def objective_reversible(par, fuga, temperature, Tave, rMeOH_exp, rCO_exp):

    try:
        rMeOH_pred, rCO_pred = calc_predictions_reversible(
            par,
            fuga,
            temperature,
            Tave
        ...
        print("objective_reversible 出错:", e)
        return 1e30
...
def make_single_parity_plot(
    ...
    plt.scatter(exp, pred, alpha=0.75, label=label)
...
# =========================
# 9. 拟合 reversible power law
# =========================
def fit_reversible_model(df_group):

    model_name = "reversible_powerlaw_free_reverse"
    ...
    bounds = [
        (-30, 30),      # ln_k1f_ref
        (0, 150000),    # E1f
        (-2, 5),        # n1_A, CO2 in forward MeOH formation
        (-2, 5),        # n1_B, H2 in forward MeOH formation
        (-2, 5),        # n1_C, CH3OH in reverse MeOH reaction
        (-2, 5),        # n1_D, H2O in reverse MeOH reaction

        (-30, 30),      # ln_k2f_ref
        (0, 150000),    # E2f
        (-2, 5),        # n2_A, CO2 in forward RWGS
        (-2, 5),        # n2_B, H2 in forward RWGS
        (-2, 5),        # n2_D, CO in reverse RWGS
        (-2, 5)         # n2_E, H2O in reverse RWGS
    ]

    result = differential_evolution(
        objective_reversible,
        bounds=bounds,
    ...
    k1f_ref_at_Tave = np.exp(np.clip(ln_k1f_ref, -700, 700))
    ...
    params = {
        ...
        "ln_k1f_ref": ln_k1f_ref,
        "k1f_ref_at_Tave": k1f_ref_at_Tave,
        "E1f_J_per_mol": E1f,
        "E1f_kJ_per_mol": E1f / 1000.0,
        "n1_A": n1_A,
        "n1_B": n1_B,
        "n1_C": n1_C,
        "n1_D": n1_D,
...
# 10. 拟合单个 GHSV 和 p 分组
# =========================
def fit_one_group(df_group, ghsv_value, p_value):

    ghsv_label = int(round(float(ghsv_value)))

    p_label = float(p_value)
    if abs(p_label - round(p_label)) < 1e-12:
        p_label_for_file = int(round(p_label))
    else:
        p_label_for_file = p_label

    group_dir = OUT_DIR / f"GHSV_{ghsv_label}_p_{p_label_for_file}"
    group_dir.mkdir(exist_ok=True)

    print("\n" + "=" * 70)
    print(f"开始拟合 GHSV = {ghsv_label}, p = {p_label_for_file}")
    print(f"该组数据点数 = {len(df_group)}")
    print(f"Tave = {df_group['T'].mean():.2f} K")
    print("=" * 70)

    # beta 诊断保留，用于判断接近平衡程度
    ...
    # 只拟合 reversible power law
    summary_reversible, params_reversible, pred_reversible = fit_reversible_model(
        df_group
    )

    summary_df = pd.DataFrame([summary_reversible])
    params_df = pd.DataFrame([params_reversible])

    result_df = df_group.copy()

    for col in pred_reversible.columns:
        result_df[col] = pred_reversible[col].values

    # 保存单个组结果
    summary_df.to_excel(
        group_dir / f"GHSV_{ghsv_label}_p_{p_label_for_file}_summary.xlsx",
        index=False
    )

    params_df.to_excel(
        group_dir / f"GHSV_{ghsv_label}_p_{p_label_for_file}_parameters.xlsx",
        index=False
    )

    result_df.to_excel(
        group_dir / f"GHSV_{ghsv_label}_p_{p_label_for_file}_predictions.xlsx",
        index=False
    )
    ...
    make_single_parity_plot(
        ...
        pred=result_df["reversible_powerlaw_free_reverse_rMeOH_pred"].values,
        label="reversible_powerlaw_free_reverse",
        xlabel="Experimental MeOH",
        ylabel="Predicted MeOH",
        title=f"MeOH, GHSV = {ghsv_label}, p = {p_label_for_file} MPa",
        save_path=group_dir / f"GHSV_{ghsv_label}_p_{p_label_for_file}_parity_meoh.png"
    )

    # 画 CO parity plot
    make_single_parity_plot(
        exp=result_df["rCO"].values,
        pred=result_df["reversible_powerlaw_free_reverse_rCO_pred"].values,
        label="reversible_powerlaw_free_reverse",
        xlabel="Experimental CO",
        ylabel="Predicted CO",
        title=f"CO, GHSV = {ghsv_label}, p = {p_label_for_file} MPa",
        save_path=group_dir / f"GHSV_{ghsv_label}_p_{p_label_for_file}_parity_co.png"
    )

    summary_df.insert(0, "p", p_label_for_file)
    summary_df.insert(0, "GHSV", ghsv_label)

    params_df.insert(0, "p", p_label_for_file)
    params_df.insert(0, "GHSV", ghsv_label)

    result_df.insert(0, "fit_p", p_label_for_file)
    result_df.insert(0, "fit_GHSV", ghsv_label)
...
def main():
    ...
    for ghsv in ghsv_values:
        ...
        for p_value in p_values:
            ...
            df_group = df[
                np.isclose(df["GHSV"], ghsv)
                & np.isclose(df["p"], p_value)
            ].copy()

            if len(df_group) < 8:
                print(
                    f"GHSV = {ghsv}, p = {p_value} 的数据点太少，跳过。"
                    f" 当前数据点数 = {len(df_group)}"
                )
                continue

            summary_df, params_df, result_df, beta_summary = fit_one_group(
                df_group,
                ghsv,
                p_value
            )
    ...
    all_summary_df.to_excel(
        OUT_DIR / "all_GHSV_p_summary.xlsx",
        index=False
    )

    all_params_df.to_excel(
        OUT_DIR / "all_GHSV_p_parameters.xlsx",
        index=False
    )

    all_predictions_df.to_excel(
        OUT_DIR / "all_GHSV_p_predictions.xlsx",
        index=False
    )

    all_beta_summary_df.to_excel(
        OUT_DIR / "all_GHSV_p_beta_summary.xlsx",
        index=False
    )
