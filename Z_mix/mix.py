# 按 GHSV 分类，只拟合 hybrid model
#
# Hybrid model:
#   反应 1 MeOH formation:
#       rMeOH = k1 * fCO2^n1_A * fH2^n1_B * (1 - beta1)
#
#   反应 2 RWGS:
#       rCO = rf - rr
...
OUT_DIR = Path("R_GHSV_cluster_hybrid_only")
...
# =========================
def calc_predictions_hybrid(par, fuga, temperature, Tave):
    ...
    # 反应 1:
    # MeOH formation 使用 power law × (1 - beta1)
...
# 用相对误差平方和
# =========================
def objective_hybrid(par, fuga, temperature, Tave, rMeOH_exp, rCO_exp):

    try:
        rMeOH_pred, rCO_pred = calc_predictions_hybrid(
            par,
            fuga,
            temperature,
            Tave
...
def make_parity_plot(exp, pred, xlabel, ylabel, title, save_path):
    ...
    plt.scatter(exp, pred, alpha=0.75, label="hybrid_model")
...
# 9. 拟合 hybrid model
# =========================
def fit_model_hybrid(df_group):

    model_name = "hybrid_meoh_beta_rwgs_reversible"
...
def fit_one_ghsv_group(df_group, ghsv_value):
    ...
    summary_hybrid, params_hybrid, pred_hybrid = fit_model_hybrid(
        df_group=df_group
    )

    summary_df = pd.DataFrame([summary_hybrid])
    params_df = pd.DataFrame([params_hybrid])

    result_df = df_group.copy()

    for col in pred_hybrid.columns:
        result_df[col] = pred_hybrid[col].values

    summary_df.to_excel(
        group_dir / f"GHSV_{ghsv_label}_hybrid_summary.xlsx",
        index=False
    )

    params_df.to_excel(
        group_dir / f"GHSV_{ghsv_label}_hybrid_parameters.xlsx",
        index=False
    )

    result_df.to_excel(
        group_dir / f"GHSV_{ghsv_label}_hybrid_predictions.xlsx",
        index=False
    )

    make_parity_plot(
        exp=result_df["rMeOH"].values,
        pred=result_df["hybrid_meoh_beta_rwgs_reversible_rMeOH_pred"].values,
        xlabel="Experimental MeOH",
        ylabel="Predicted MeOH",
        title=f"Hybrid Model Parity Plot for MeOH, GHSV = {ghsv_label}",
        save_path=group_dir / f"GHSV_{ghsv_label}_hybrid_parity_meoh.png"
    )

    make_parity_plot(
        exp=result_df["rCO"].values,
        pred=result_df["hybrid_meoh_beta_rwgs_reversible_rCO_pred"].values,
        xlabel="Experimental CO",
        ylabel="Predicted CO",
        title=f"Hybrid Model Parity Plot for CO, GHSV = {ghsv_label}",
        save_path=group_dir / f"GHSV_{ghsv_label}_hybrid_parity_co.png"
    )
...
def main():
    ...
    all_summary_df.to_excel(
        OUT_DIR / "all_ghsv_hybrid_summary.xlsx",
        index=False
    )

    all_params_df.to_excel(
        OUT_DIR / "all_ghsv_hybrid_parameters.xlsx",
        index=False
    )

    all_predictions_df.to_excel(
        OUT_DIR / "all_ghsv_hybrid_predictions.xlsx",
        index=False
    )
